...
import ChannelsList from '@/components/admin/ChannelsList.vue'
...
const components = [
  { name: 'Usuarios', icon: Users, component: UsersList },
  { name: 'Canales', icon: Radio, component: ChannelsList },
  { name: 'Permisos', icon: Shield, component: PermissionsList },
  { name: 'Roles', icon: UserCog, component: RolesList },
  { name: 'Historial', icon: History, component: HistoryList }
]
</script>
        ...
        <nav>
          <ul class="space-y-2">
            <li v-for="component in components" :key="component.name">
              <a href="#" @click="currentComponent = component.name" :class="[
